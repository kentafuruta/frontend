...
gulp.task('ejs', function() {
  var jsonData = './src/ejs/pages.json',
      template = './src/ejs/**/!(_)*.ejs',
      json = JSON.parse(fs.readFileSync(jsonData)),
      pages = json.pages;

  for (var i = 0; i < pages.length; i++) {
    var id = pages[i].id;
    var childs = pages[i].childs;
    ...
    if('childs' in pages[i]) {
      for (var c = 0; c < childs.length; c++) {
        gulp.src(template)
          ...
          .pipe(rename(childs[c].id + ".html"))
          .pipe(gulp.dest('dist/' + id + '/'));
      }
    }
  }
});
